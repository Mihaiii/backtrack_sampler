...
class AdaptiveTemperatureStrategy(BaseStrategy):
    def __init__(self, poly_coeffs: torch.Tensor = torch.tensor([-0.037, 0.481, -2.3, 4.917, -1.791])):
    ...
    def adaptive_temperature_softmax(self, probs, logits):
        ...
        beta = (self.poly_coeffs[0] * x**4 + self.poly_coeffs[1] * x**3 + self.poly_coeffs[2] * x**2 + self.poly_coeffs[3] * x + self.poly_coeffs[4])
        if x <= 0.5:
            beta = 1.0
        else:
            beta = max(beta, 1.0)
